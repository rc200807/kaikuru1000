...
| Phase 3 画像/ファイル | ✅ 本番反映 | `cbf2ab6` | アップロード画像をWebP化（実データで68〜98%削減）＋サムネ生成、`<img>`132箇所を遅延読み込み |
| Phase 4 バンドル削減 | ✅ 本番反映 | `f7b2939` | 公開ページのモバイルJS **291KB→159KB（-45%)**、未使用依存3つ削除 |
| Phase 5 レンダリング方式 | 一部反映 | `f7b2939` `3b48aab` | 店舗・管理レイアウトのサーバーコンポーネント化＋`loading.tsx` 骨組み |
| 管理ポータルへの横展開 | ✅ 本番反映 | `3b48aab` | ナビのバッジ集約（チャット未読のN+1解消）・ダッシュボード18本のクエリを並列化 |
| 既存画像の一括WebP化 | ✅ 本番反映 | `6aed634` `d979814` `993ae96` | 管理 > 設定 > 既存画像の最適化（確認モードあり・中断/再開可） |
| Google Analytics 廃止 | ✅ 本番反映 | `6aed634` | CSPでブロックされ続けていたgtagを撤去。設定画面と公開APIも削除 |
| Phase 6 インフラ | 未着手（判断待ち） | | 関数＋DBのリージョン移設。効果は大きいが移設判断が必要 |

### 運用メモ: 既存画像の一括WebP化

`管理ポータル > 設定 > 既存画像の最適化`（スーパー管理者のみ）。

1. まず「確認モード」で実行する（DBもストレージも変更せず、対象枚数と削減見込みだけ出す）
2. 問題なければチェックを外して「変換を実行」。途中で閉じても、再実行すれば続きから進む
3. 元ファイルは残るので、表示がおかしくなった場合はDBのURLを戻せば元に戻せる
4. 画面上部に赤い警告が出る場合は画像変換ライブラリ（sharp）が動いていない。その状態では
   変換されないので、実行せずに調査すること

**注意（実際に踏んだ問題）**: sharp はネイティブライブラリ（libvips）に依存する。
Next.js の出力トレースは `@img/sharp-libvips-*` の JS だけ拾い、実体の `.so` を
関数に同梱しないため、本番だけ `ERR_DLOPEN_FAILED` で画像アップロードが500になる。
`next.config.ts` の `outputFileTracingIncludes` で `./node_modules/@img/**/*` を
明示的に含めること。

### 本番で確認できた効果（日本・モバイル 375px / 顧客ログインページ）

| | 変更前 | 変更後 |
|---|---|---|
| JS転送量 | 291KB（three.js込み） | **159KB**（three.js無し・CSS背景） |
| WebGL描画 | あり（スマホでも） | なし（1440pxでは従来どおり3D） |

画像は 5.9MB のPNGをアップロードして 本体18KB（2000x1333）＋サムネ1.2KB（400x267）で保存されることを確認。
