  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        {/* Notification banner for shipped */}
        {shippedCount > 0 && (
        ...
        {loading ? (
          ...
          <Card variant="elevated" padding="none">
            <EmptyState title="該当する宅配買取がありません" />
          </Card>
        ) : (
          <div className="space-y-3">
            <p className="text-sm text-[var(--md-sys-color-on-surface-variant)]">
              {records.length}件の送付記録
            </p>
            {records.map(rec => (
              <Card
                key={rec.id}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-0.5 text-xs text-[var(--md-sys-color-on-surface-variant)]">
                      ...
                      {rec.description && (
                        <span className="truncate max-w-48">{rec.description}</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="text-right shrink-0">
                    {rec.purchaseAmount != null ? (
                      <p className="text-sm font-bold text-[var(--md-sys-color-on-surface)]">
                        ¥{rec.purchaseAmount.toLocaleString()}
                      </p>
                    ) : (
