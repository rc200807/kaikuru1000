        ...
        {!loading && statuses.length > 0 && (
          <div className="space-y-1">
            {statuses.map((s: any) => (
              <div
                key={s.id}
                className="flex items-center gap-3 py-2.5 px-3 rounded-[var(--md-sys-shape-small)] hover:bg-[var(--md-sys-color-surface-container-low)] transition-colors"
              >
                <span
                  className="w-3 h-3 rounded-full flex-shrink-0"
                  style={{ backgroundColor: s.color }}
                />
                <span className="text-sm font-medium text-[var(--md-sys-color-on-surface)] flex-1">
                  {s.label}
                </span>
                <span className="text-xs text-[var(--md-sys-color-on-surface-variant)] bg-[var(--md-sys-color-surface-container-high)] px-2 py-0.5 rounded-full flex-shrink-0 font-mono">
                  {s.key}
                </span>
                <Button variant="text" size="sm" onClick={() => handleEdit(s)}>
                  編集
                </Button>
                {!SYSTEM_KEYS.includes(s.key) && (
                  <Button variant="text" size="sm" danger onClick={() => handleDelete(s)}>
                    削除
                  </Button>
                )}
              </div>
          ...
          <form onSubmit={handleSubmit} className="max-w-lg space-y-4 p-4 rounded-[var(--md-sys-shape-medium)] border border-[var(--md-sys-color-outline-variant)] bg-[var(--md-sys-color-surface-container-low)]">
            ...
            {!editingId && (
              <TextField
                ...
                value={formKey}
                onChange={setFormKey}
                placeholder="例: follow_up"
                required
                helper="半角英数字とアンダースコアのみ使用できます"
              />
            )}
            {editingId && (
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-8 h-8 rounded cursor-pointer border border-[var(--md-sys-color-outline-variant)]"
                />
                <input
                  type="text"
                  value={formColor}
                  onChange={e => setFormColor(e.target.value)}
                  className="text-sm font-mono px-2 py-1 border border-[var(--md-sys-color-outline-variant)] rounded-[var(--md-sys-shape-small)] bg-[var(--md-sys-color-surface-container-lowest,#fff)] w-24"
                  placeholder="#000000"
                />
              </div>
            </div>
            <TextField
              ...
              onChange={v => setFormSortOrder(parseInt(v) || 0)}
              type="number"
            />
          ...
          <div className="space-y-1">
            {categories.map((cat: any) => (
              ...
              >
                ...
                <Button variant="text" size="sm" onClick={() => handleEdit(cat)}>
                  編集
                </Button>
                <Button variant="text" size="sm" danger onClick={() => handleDelete(cat)}>
                  削除
                </Button>
              </div>
            ))}
