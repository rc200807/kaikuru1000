      ...
      <div className="flex items-center justify-around max-w-lg mx-auto h-16">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div className={`flex items-center justify-center w-8 h-8 rounded-2xl transition-all ${
                active ? 'bg-red-50' : ''
              }`}>
                <span className={active ? 'text-[#B91C1C]' : 'text-gray-500'}>
                  {active ? item.iconFilled : item.icon}
                </span>
              </div>
              <span className={`text-[10px] font-medium leading-tight ${
                active ? 'text-[#B91C1C]' : 'text-gray-500'
              }`}>
                {item.label}
              </span>
            </button>
