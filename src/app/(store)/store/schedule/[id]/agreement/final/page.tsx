...
export default function FinalAgreementPage() {
  ...
  useEffect(() => {
    ...
    if (!visit.user.idDocumentPath) {
      const qs = staffName ? `?staff=${encodeURIComponent(staffName)}` : ''
      router.replace(`/store/schedule/${scheduleId}/agreement${qs}`)
    }
  }, [loading, visit, staffName, scheduleId, router])
  ...
  if (!visit) {
    return (
      <div className="p-6">
        ...
        <Button variant="text" onClick={backToAgreement} className="mt-4">← 戻る</Button>
      </div>
    )
  }
  ...
  const invoiceNo = `INV-${scheduleId.slice(-8).toUpperCase()}`
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-6 space-y-5">
      ...
      <div className="flex items-center gap-3">
        <button
          onClick={backToAgreement}
          className="text-[var(--portal-primary)] hover:underline text-sm"
        >
          ← 取引内容を編集
        </button>
        <h1 className="text-lg font-bold text-[var(--md-sys-color-on-surface)] flex-1">売買契約書・請求書</h1>
        {pinLocked && (
          <button
            onClick={() => {
              pendingNavigationRef.current = null
          ...
          {magicLinkUrl ? (
            ...
            <div className="flex flex-col items-center gap-3">
              {magicLinkLoading ? (
                ...
                <Button variant="outlined" onClick={generateMagicLink}>マジックリンクを生成</Button>
              )}
            </div>
          )}
        </Card>
      ...
      <div ref={saleRef} className="space-y-5 bg-white p-1 rounded-xl">
        ...
        <Card variant="elevated" padding="md">
          <div className="flex items-baseline justify-between mb-3 pb-2 border-b-2 border-[var(--portal-primary)]">
            <h2 className="text-base font-bold text-[var(--md-sys-color-on-surface)]">売買契約書</h2>
            <span className="text-[10px] text-[var(--md-sys-color-on-surface-variant)]">契約番号: {contractNo}</span>
          </div>
        ...
        <Card variant="elevated" padding="md">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {generatedPdfs.invoice && (
              <Button variant="tonal" onClick={() => openPdfBase64(generatedPdfs.invoice!)}>請求書PDFを確認する</Button>
            )}
          </div>
        </Card>
      )}
