...
import { exportCustomersToSheet, importCustomersFromSheet, extractSpreadsheetId } from '@/lib/sheet-sync'
...
export async function PUT(req: NextRequest) {
  ...
  const sheetName = (typeof body.sheetName === 'string' ? body.sheetName.trim() : '') || '顧客情報'
  ...
  if (existing) {
    await prisma.googleSheetsConfig.update({
      ...
      data: { customerSpreadsheetId: spreadsheetId, customerSheetName: sheetName },
    })
  } else {
...
export async function POST(req: NextRequest) {
  ...
  if (action === 'export') {
    const result = await exportCustomersToSheet()
    if (result.success) {
      await recordAccessLog({
        userType: user.role, userId: user.id, userName: user.name,
        action: `顧客情報をスプレッドシートへ出力（${result.exported}件）`, req,
      })
    }
  ...
  if (action === 'import') {
    const result = await importCustomersFromSheet()
    if (result.success) {
      await recordAccessLog({
        userType: user.role, userId: user.id, userName: user.name,
        action: `顧客情報をスプレッドシートから同期（新規${result.createdCount}・更新${result.updatedCount}・エラー${result.errorCount}）`, req,
      })
    }
    return NextResponse.json(result, { status: result.success ? 200 : 400 })
  }
