...
/**
 * 定期宅配の発送伝票画像を認証プロキシ経由で配信
 * Blob URL をクライアントに露出させず、認証・認可チェック後にコンテンツを返す
 *
 * GET /api/delivery-shipments/[id]/tracking-images/[index]
 *   - 顧客: 自分の送付のみ
 *   - 店舗: 担当顧客の送付のみ
 *   - 管理者: すべて
 */
export async function GET(
  ...
  // trackingImageUrls は DB に JSON 文字列で保存されている
  let blobUrls: string[] = []
  try { blobUrls = JSON.parse(shipment.trackingImageUrls || '[]') } catch { /* ignore */ }
