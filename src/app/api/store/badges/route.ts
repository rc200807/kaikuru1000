 ...
 *
 * 【表示スコープ非対応は意図的】
 * 運営者配下の複数店舗を表示中でも、バッジは常にログイン中の店舗のみを数える。
 * バッジは「見えている範囲」ではなく「自分の受信箱」だから:
 *  - 既読はセッション店舗（またはメンバー）に紐づくので、他店舗ぶんを数えても
 *    この画面から既読にする手段がなく、永久に光り続けるバッジになる
 *  - 訪問リクエストも承認できるのは自店舗ぶんだけ（api/visit-requests/[id] の帰属チェック）
 *  - ここは 60 秒ポーリング＋フォーカス再取得のホットパス（StoreBadgesContext）
 * 「直し忘れ」ではないので storeIds 対応を足さないこと。
 */
