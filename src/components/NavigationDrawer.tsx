      ...
      <div className="relative border-t border-[var(--md-sys-color-outline-variant)]" ref={userMenuRef}>
        ...
        >
          {user?.avatar ? (
            ...
            <div className="w-9 h-9 rounded-full bg-[var(--admin-primary)] flex items-center justify-center shrink-0">
              <span className="text-[var(--admin-on-primary)] text-sm font-semibold">{user?.name?.[0] ?? '?'}</span>
            </div>
          )}
          <div className="min-w-0 flex-1 text-left">
            <p className="text-sm font-medium text-[var(--md-sys-color-on-surface)] truncate">{user?.name ?? '管理者'}</p>
