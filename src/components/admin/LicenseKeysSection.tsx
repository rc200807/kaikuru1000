...
/**
 * ライセンスキー管理セクション。
 * `/admin/partners` のタブ内、および旧 `/admin/licenses` ページから利用される。
 */
export default function LicenseKeysSection() {
  ...
  async function handleImportFromSheets() {
    ...
    if (res.ok && data.success) {
      setMessage({ type: 'success', text: `インポート完了: ${data.created}件追加（${data.skipped}件は重複スキップ）` })
      fetchKeys()
    } else {
  ...
  async function handleImportCSV(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (res.ok && data.success) {
      setMessage({ type: 'success', text: `CSVインポート完了: ${data.created}件追加（${data.skipped}件は重複スキップ）` })
      fetchKeys()
    } else {
  ...
  if (loading) {
    return <div className="flex justify-center py-16"><LoadingSpinner size="lg" label="読み込み中..." /></div>
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <h2 className="text-base font-semibold text-[var(--md-sys-color-on-surface)]">ライセンスキー</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <SummaryCard label="合計" value={keys.length} unit="件" accentColor="bg-[var(--portal-primary,#374151)]" />
        <SummaryCard label="未使用" value={unusedCount} unit="件" accentColor="bg-[var(--status-completed-text)]" />
        <SummaryCard label="使用済み" value={usedCount} unit="件" accentColor="bg-[var(--md-sys-color-outline)]" />
      </div>
