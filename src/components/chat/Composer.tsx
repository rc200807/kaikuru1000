...
export default function Composer({
  ...
  const empty = isEmptyChatHtml(html)
  const canSend = !sending && !uploading && (!empty || pending.some((p) => !p.uploading && p.url))
  ...
  const send = async () => {
    ...
    if ((empty && ready.length === 0) || sending || uploading) return
    setSending(true)
    try {
      await onSend(
        empty ? '' : html,
