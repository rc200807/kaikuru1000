...
const CATEGORY: Record<string, { label: string; icon: string; color: string }> = {
  feature: { label: '新機能', icon: '🚀', color: '#3B82F6' },
  improvement: { label: '改善', icon: '✨', color: '#10B981' },
  fix: { label: '修正', icon: '🛠️', color: '#F59E0B' },
  notice: { label: 'お知らせ', icon: '📣', color: '#8B5CF6' },
}
  ...
  return (
    <section className="rounded-2xl border border-[var(--md-sys-color-outline-variant)] bg-[var(--md-sys-color-surface-container-low)] p-5">
      <div className="flex items-center gap-2 mb-3">
        <h2 className="text-sm font-semibold text-[var(--md-sys-color-on-surface)]">📣 アップデート情報</h2>
        {unreadCount > 0 && (
      ...
      <div className="divide-y divide-[var(--md-sys-color-outline-variant)]">
        {visible.map(n => {
          const cat = CATEGORY[n.category] ?? CATEGORY.feature
